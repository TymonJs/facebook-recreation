...
export default function Messenger({ friend = "", loggedLogin = "", selfRef }) {
  ...
  const {
    ...
    image = "",
    login = "",
  } = friend.user ? friend.user : friend;
  ...
  const cancelIcon = (
    <i className="fa-solid fa-ban" onClick={() => cancelEdit()}></i>
  );
          ...
          <>
            <div className="icons">
              <i
                ...
                onClick={() => removeMsg(key ? key : i)}
              ></i>
              <i
                className="fa-solid fa-pen"
                onClick={() => editMsg(key ? key : i)}
              ></i>
            </div>
          </>
        ) : null;
      return (
        <div className={`message-row ${by}${spaced}`} key={key ? key : i}>
          {icons}
          {msg}
        </div>
      );
  ...
  useEffect(() => {
    if (login) {
      ...
      if (friend.messages) {
        setLoading();
        setMessages([...friend.messages].reverse());
      }
      if (!selfRef.current.classList.contains("active"))
        selfRef.current.classList.add("active");
      fetch(
        `${window.location.origin}/api/message/${login}/${loggedLogin}`,
      ).then((r) => {
        if (r.ok) {
